...
// Component to display a modal for group details
function Model(props) {
  ...
  // Handle click on a search result to add user to the group
  ...
  // Update group details (name) on button click
  ...
  // Set initial members when the active chat changes
  useEffect(() => {
    setMembers(activeChat?.users.map((e) => e));
  }, [activeChat]);

  // Update search results based on the search input
  useEffect(() => {
  ...
  return (
    <>
      ...
      {activeChat?.isGroup ? (
        ...
        >
          <Box sx={style}>
            <h5 className='text-[22px] font-semibold tracking-wide text-center text-[#111b21]'>
