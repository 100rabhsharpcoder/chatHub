...
function Search({ type, isLoading, searchResults, handleClick, search }) {
  return (
    <div className={`${search ? 'scrollbar-hide overflow-y-scroll h-[250px] mb-5 bg-[#fff] flex flex-col gap-y-3 pt-3' : 'hidden'}`}>
      {/* Check if loading is in progress, show skeleton loading */}
      {isLoading ? (
        <SkeletonLoading height={55} count={3} />
      ) : (
        // If there are search results, map through them and display each result
        searchResults.length > 0 ? (
          searchResults?.map((e) => {
            return (
              <div key={e._id} className='flex items-center justify-between'>
                <div className='flex items-center gap-x-2'>
                  {/* Display user profile picture, if not available, use a default avatar */}
                ...
                {/* Button to handle the click event, e.g., add friend */}
                <button onClick={() => handleClick(e)} className='bg-[#0086ea] px-3 py-2 text-[10.6px] tracking-wide text-[#fff]'>
                  Add
                </button>
              </div>
            );
